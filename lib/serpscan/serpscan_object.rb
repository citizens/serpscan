  ...
  class SerpscanObject
    ...
    class << self
      ...
      def all(options = {})
        ...
        results = API.get(url, params: options[:params])
       
        results['results'].map do |object| 
          objects << self.new(object.merge(parent: options[:parent])) 
        end
      ...
      def create(params)
        ...
        self.new(results.merge(parent: parent_object))
      end
    ...
    def assign_attributes(options = {})
      self.class::ATTRIBUTES.each do |attribute|
        self.send("#{attribute}=", options[attribute.to_s] )
      end
    end
  end
end
